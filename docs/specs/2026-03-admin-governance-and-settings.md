...
- Includes: invoice, `vat_rate_percent` (`invoice.vat_rate × 100`, percent form for the VAT label — `vat_rate` itself stays a fraction), items grouped by category (`grouped_items`), zev, participant, owner_participant, QR-Rechnung SVG, `inline_qr_payment` flag, invoice-number prefix/suffix split, localized `status_display`, energy comparison chart SVG, hourly profile chart SVG, energy-flow Sankey SVG, energy summary KPIs, savings data, and the translation dict (`tr`)
...
- A `FieldReference` sidebar lists the available context variables per template type (invoice, participant, ZEV, owner, line-item loops, charts/savings, translations). For invoice VAT, it distinguishes the stored fractional `invoice.vat_rate` (for example `0.0810`) from the display-ready `vat_rate_percent` (for example `8.1`).
