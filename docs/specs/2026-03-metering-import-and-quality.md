...
unknown ZEV (404), foreign-owned ZEV (403, non-admin), or writes to a disabled
ZEV (400, non-admin) before parsing. Admins may import into disabled ZEVs;
owners retain read-only CSV preview access.
...
`settings` uses the same names/values the import endpoints take (`column_map` entries are a unique header label, else the column index as a string; `null` = not applicable/not found). `undetected` lists what could not be determined (`meter_id`, `timestamp`, `energy_kwh`, `timestamp_format`, or `file` when there are fewer than two rows); `detected` is false when anything but `timestamp_format` is missing. Rules:

- **Delimiter** (CSV only; XLSX reports `,`): candidates `; , \t |` are split over the first 30 lines of the first 64 KB; a candidate needs ≥ 2 columns on ≥ 80 % of lines, the widest split wins and `;` breaks ties (so a `;` file with decimal commas is not read as `,`). Default `,`.
- **Header row:** the first row is a header unless it contains a date-like cell or a decimal number (numeric labels such as `1, 2, 3` are still a header).
- **Cell kinds** (a column's kind is the one covering ≥ 80 % of its non-empty sampled cells): `date` (`YYYY-MM-DD…`/`DD.MM.YYYY…` text or a native datetime), `time` (`HH:MM`), `direction` (`in`/`out` or an OBIS code), `number` (parseable by the importer's own `_parse_decimal`, so decimal commas count), else `text`.
- **Columns:** timestamp = a `date` column (header hint `date|datum|time|zeit|stamp|tag|day` wins among several); direction = the first `direction` column; meter id = a `text` column (all-digit `number` columns only as a fallback), header hint `meter|zähl|messpunkt|metering|mp|pod|ean` wins; energy = a `number` column (hint `kwh|energy|energie|wert|value|verbrauch|menge|consum|reading` wins, else the first). Hints only rank candidates whose contents already fit.
- **Row layout:** `daily_15min` when ≥ 12 header labels are times (`00:00`, `00:15`, …) or a run of ≥ 12 consecutive `number` columns follows; `values_count` is the run length (a run one longer than 24/48/96/144/288/1440 has its last column dropped as a day total) and `interval_minutes` is the step between the first two time labels, else `1440 / values_count` when it divides evenly, else 15. Otherwise `standard`.
- **Timestamp format:** `""` (auto-detect) for native datetimes and ISO-8601 text; else the first of a fixed list of `strptime` formats (`%d.%m.%Y[ %H:%M[:%S]]`, `%d.%m.%y…`, `%d/%m/%Y…`, `%d-%m-%Y…`, `%Y/%m/%d…`, `%m/%d/%Y…`, day-first before month-first) that parses every sampled cell and passes `_check_timestamp_format`; if none does, `""` plus `timestamp_format` in `undetected`.
...
- `backend/accounts/throttling.py:ImportThrottle` (`UserRateThrottle` subclass, `600/hour` per-user) on `POST /import/csv`, `/import/sdatch`, `/import/preview-csv`, `/import/detect-csv` (`backend/metering/views.py`). The budget is sized for multi-file wizard runs: one file costs up to three requests (detect once per selection, preview and import per file), so a large ZEV can bring in a few hundred files per hour; `backend/accounts/throttling.py:TransferArchiveThrottle` (`UserRateThrottle` subclass, `20/hour` per-user) on `POST /zev/zevs/import-archive` and `inspect-archive` (`backend/zev/views.py`).
...
| `SdatchImportTests` (ZEV validation) | §4.2/5.6: SDAT-CH import rejects missing `zev_id` (400), malformed UUID (400), and unknown ZEV (404); foreign-owned ZEV → 403; disabled ZEV for its owner → 400; admins can import into disabled ZEVs |
...
- Wizard step 2 (CSV) is grouped into `form-section` blocks with the shared `form-section-header` style: **File format** (row format, delimiter, date/time format, header-row checkbox), **Column mapping** (meter ID, date/timestamp, energy column or first-interval column, then intervals per row and minutes per interval for daily profiles, and the optional direction column last so its long hint cannot stretch a row; it spans two columns in the standard layout via `.grid-span-2`, which only applies above 640 px), **Options** (overwrite, with a hint that it cannot be undone) and **Preview**. The target ZEV is a read-only line, not a disabled input. Locale keys: `pages.imports.wizard.targetZev`, `sections.*`, `overwriteExistingHint`.
