...
|---|---|---|---|---|
| Dashboard | `frontend/src/pages/DashboardPage.tsx` (246 lines; cards in `frontend/src/components/dashboard/`: `BalanceChart`, `ConsumptionSplitCard`, `EnergyFlowCard`, `HourlyProfileCard`, `ParticipantInvoicesCard`) | manager `/dashboard` (Energy balance) + participant `/` | `queryKeys.metering.dashboardSummary({…})` + participant-only `queryKeys.invoices.list()` + participant-only `queryKeys.metering.hourlyProfile(…)` | none — read-only. Manager: ZEV-wide KPI row (5 `StatCard`s from `zev_totals`) → `EnergyFlowCard` → `BalanceChart` (both keep `selectedParticipantId` highlight/filter) → period-preserving link to `/metering/chart?period_start&period_end` (per-participant table and manager hourly profile removed; per-meter charts live on Metering). Participant: 3 `StatCard`s → `EnergyFlowCard` → `ConsumptionSplitCard` → `HourlyProfileCard` → `ParticipantInvoicesCard` (role-scoped `fetchInvoices` filtered to `approved|sent|paid` with `pdf_url`, details + PDF actions). Header eyebrow is the scope line: selected ZEV name for owners/admins, the participant's community name (`zev_name`, only with a single membership, `zev_count === 1`) from `/auth/me` for participants; blank with no selection or several memberships. No document-download controls (those live on Reports). |
