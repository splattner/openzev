...
| Integrated backup and restore — scheduled encrypted backups to local/S3, whole-instance recovery, per-ZEV rollback | `in-progress` | `high` | [spec](specs/2026-09-backup-and-restore.md), [ADR 0023](adr/0023-backup-archives-preserve-keys.md), [ADR 0024](adr/0024-backup-encryption-key.md), #767. Phases 1 (encrypted backups to local/S3, verify, admin UI) and 2 (whole-instance restore from the command line, fresh-install bootstrap) done; per-ZEV restore, scheduling and retention follow. Replaces the `pg_dump`-only guidance, which omits `MEDIA_ROOT` |
