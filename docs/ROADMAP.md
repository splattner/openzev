...
| Dynamic tariffs — fetched quarter-hourly price series (VSE-compatible v1.0.5/v2.0.0, provider-neutral capability discovery), engine resolution with a hard refusal on gaps, category-agnostic readiness coverage, VSE import and a manual two-step creation wizard, fetched-price history (chart/stats/gaps), and a global admin operations console (refresh/backfill/recheck/edit/clear/delete, guarded and audited) | `shipped` | — | [spec](specs/2026-09-dynamic-tariffs.md), [ADR 0018](adr/0018-dynamic-tariff-price-series.md), [guide](user-guide/07-tariff-configuration.md#dynamic-tariffs), #530, #702, #703 |
...
| Integrated backup and restore — scheduled encrypted backups to local/S3, whole-instance recovery, per-ZEV rollback | `in-progress` | `high` | [spec](specs/2026-09-backup-and-restore.md), [ADR 0023](adr/0023-backup-archives-preserve-keys.md), [ADR 0024](adr/0024-backup-encryption-key.md), #767. Phases 1 (encrypted backups to local/S3, verify, admin UI), 2 (whole-instance restore from the command line, fresh-install bootstrap) and 3 (per-ZEV restore: plan, safety backup, API and admin UI) done; scheduling and retention follow. Replaces the `pg_dump`-only guidance, which omits `MEDIA_ROOT` |
