...
Billing resolves the price at each reading's own timestamp, so DST days need no special handling and a levy charged as a percentage of the grid rate follows the dynamic rate with no extra configuration. Prices are stored rather than cached, because operators keep little history of their own — Groupe E drops it after about nine months, BKW publishes none at all — so an interval nobody stored on the day could never be re-derived.
