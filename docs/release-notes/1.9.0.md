...
```
Netznutzung                     1050.0000 kWh × 0.22749 = 238.87   (before)

Netznutzung – HT (Hochtarif)     412.0000 kWh × 0.28400 = 117.01   (after)
Netznutzung – NT (Niedertarif)   638.0000 kWh × 0.19100 = 121.86
```

![The setting, under Settings → General ZEV Settings](screenshots/1.9.0-band-setting.png)
...
**The invoice costs exactly the same either way** — note the two lines above summing to the same 238.87. A tariff's total is worked out first and then divided across its band lines, never the other way round, so switching the setting on cannot move anyone's bill by a centime.
