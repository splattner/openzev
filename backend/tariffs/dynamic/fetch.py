...
def refresh_source(source: DynamicTariffSource, *, backfill: bool = False, now: datetime | None = None) -> FetchResult:
    ...
    endpoints republish it while it is running; tomorrow because day-ahead
    prices may arrive in the afternoon and there is no reason to wait a day
    to store them. Re-asking for an interval already stored is an idempotent
    upsert, so the overlap costs nothing.

    An endpoint that takes no range is simply asked once for whatever it serves.
    """
