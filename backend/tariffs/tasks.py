...
def _audit_best_effort(source, *, summary, status, metadata=None):
...
def fetch_dynamic_prices_impl(source_id, *, backfill: bool = False) -> dict:
...
def fetch_dynamic_prices(self, source_id, backfill: bool = False):
    ...
    """
    try:
        return fetch_dynamic_prices_impl(source_id, backfill=backfill)
    except TariffFetchError as exc:
